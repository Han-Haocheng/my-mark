...
- 那么成这个极限$I$为函数$f(x)$在区间$[a,b]$上的定积分(积分)
...
> - 令 $\lambda \to 0$
> - 两端同时取极限
> 	- 即得$\int_{a}^{b} f(x) \, \mathrm{d}x=\int_{a}^{c} f(x) \, \mathrm{d}x+\int_{c}^{b} f(x) \, \mathrm{d}x$
> - 这个性质表明定积分对于积分区间具有*可加性*
> - 根据定积分的补充规定
> 	- 则不论a,b,c的相对位置
> 	- 总有等式$\int_{a}^{b} f(x) \, \mathrm{d}x=\int_{a}^{c} f(x) \, \mathrm{d}x+\int_{c}^{b} f(x) \, \mathrm{d}x$成立
> - 当$a<b<c$时
> - 由于$\int_{a}^{c} f(x) \, \mathrm{d}x=\int_{a}^{b} f(x) \, \mathrm{d}x+\int_{b}^{c} f(x)\, \mathrm{d}x$
> 	- 得到$\int _{a}^{b}f(x) \, dx =\int _{a}^{c}f(x) \, dx -\int _{b}^{c}f(x) \, dx=\int _{a}^{c}f(x) \, dx +\int _{c}^{b}f(x) \, dx$
...
- 如果在区间$[a,b]$上$f(x)\geq 0$，那么$\int _{a}^{b}f(x) \, dx \geq 0,(a<b)$

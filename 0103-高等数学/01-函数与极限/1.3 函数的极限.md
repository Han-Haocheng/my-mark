...
>[!abstract]- 定理1***函数极限的唯一性***：如果极限$\lim_{x\to x_0}f(x)$存在，那么极限$\lim_{x\to x_0}f(x)$唯一

>[!abstract]- 定理2***函数极限的局部有界性***：如果$\lim_{ x \to x_{0} }=A$，那么存在常数$M>0$和$\delta>0$，使得当$0<|x-x_{0}|<\delta$时，有$|f(x)|\leq M$
>证明
...
>[!abstract]- 定理2推论***函数极限局部有界性的强结论***：如果$\lim_{ x \to x_{0} }=A(A\neq 0)$</br>那么存在$x_{0}$的某一去心邻域$\overset{\circ}U (x_{0})$，当$x\in \overset{\circ}U(x_{0})$时，有$|f(x)|> \frac{|A|}{2}$
>证明：函数极限的局部保号性(当A>0时)
...
>[!abstract]- 定理3***函数极限的局部保号性***：如果在$x_{0}$的某去心邻域内$f(x)\geq 0$(或$f(x)\leq 0$)，而且$\lim_{ x \to x_{0} }f(x)=A$，那么$A\geq 0$(或$A\leq 0$)

>[!abstract]- 定理4***函数与数列极限的关系***：如果极限$\lim_{ x \to x_{0} }f(x)$存在，$\{ x_{n} \}$为函数$f(x)$的定义域内任意收敛于$x_{0}$的数列，且满足$x_{n}\neq x_{0}(n\in N_{+})$，那么相应的函数值数列$\{ f(x_{n}) \}$必收敛，且$\lim_{ n \to \infty }f(x_{n})=\lim_{ x \to x_{0} }f(x)$
> 证明：函数与数列极限的关系
